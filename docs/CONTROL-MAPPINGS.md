...
Every command below is present twice in the MFD 3 profile: once for `VKB_F14_BTN7` and once for `MOZA_F16_F18_BTN3`. Both are physical activators for the same `grip-shift` semantic modifier. Hold the activator for the installed grip path while pressing the listed MFD 3 control.
...
- MOZA F-16C/F/A-18C BTN3 may continue to operate its grip function.
